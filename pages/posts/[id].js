...
import postsPageInitialPathsB30d0Resource from '../../resources/posts-page-initial-paths-b30d0'
import postsPageInitialProps200c3Resource from '../../resources/posts-page-initial-props-200c3'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Investor Solutions Manager</title>
          <meta
            property="og:title"
            content="Posts1 - Investor Solutions Manager"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsB30d0Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps200c3Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
